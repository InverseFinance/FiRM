...
contract DAIEscrowForkTest is Test {
    ...
    function setUp() public {
        ...
        vm.createSelectFork(url, 20577635);
    ...
    function testOnDeposit_successful_whenContractHoldsDAI() public {
        ...
        assertEq(
            DSR.daiBalance(address(escrow)),
            escrow.balance(),
            "DSR Balance not equal to escrow balance"
        );
    }
    ...
    function testPay_successful_whenContractHasStakedDAI() public {
        ...
        assertEq(
            DAI.balanceOf(beneficiary),
            beneficiaryBalanceBefore + 1 ether - 1
        );
    ...
    function testPay_successful_whenContractHasStakedDAIFuzz(
        uint walletAmount
    ) public {
        ...
        withinSpan(
            DAI.balanceOf(beneficiary),
            beneficiaryBalanceBefore + payAmount,
            beneficiaryBalanceBefore + payAmount - 3
        );
        assertEq(
            DSR.daiBalance(address(escrow)),
            escrow.balance(),
            "DSR balance not equal to escrow"
        );
    }
    ...
    function withinSpan(uint input, uint max, uint min) public pure {
