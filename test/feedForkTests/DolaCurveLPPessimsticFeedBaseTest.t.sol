...
abstract contract DolaCurveLPPessimsticFeedBaseTest is Test {
    ...
    ChainlinkCurveFeed coin1Fallback; // coin1 fallback if any
    ...
    function test_STALE_coin1_use_coin1_fallback_when_coin1_lt_coin2()
        public
        isFallback
    {
        // Set coin1 < coin2
        ...
        isFallback
    {
        ...
        uint256 calculatedLPUsdPrice = (feed.curvePool().get_virtual_price() *
            uint256(coin2ClFallbackPrice)) / 10 ** dolaFeed.decimals();

        assertEq(uint256(lpUsdPrice), calculatedLPUsdPrice);
        assertEq(uint256(lpUsdPrice), uint(feed.latestAnswer()));
    }

    function test_coin1FallBack_oracle() public isFallback {
        (
            uint80 roundIdFall,
            int256 coin1ClFallbackPrice,
            uint startedAtFall,
