...
import {StyCRVPriceFeed} from "src/feeds/styCRVPriceFeed.sol";
...
contract styCRVMarketForkTest is MarketForkTest {
    ...
    address escrow = 0xc06053FcAd0A0Df7cC32289A135bBEA9030C010f;
    ...
    function setUp() public {
        ...
        StyCRVPriceFeed styCRVFeed = new StyCRVPriceFeed(); 
        Market styCRVMarket = new Market(gov, lender, pauseGuardian, escrow, IDolaBorrowingRights(address(dbr)), styCRV, IOracle(address(oracle)), 5000, 5000, 1000, true);
        init(address(styCRVMarket), address(styCRVFeed));
    ...
    function testDeposit() public {
        ...
        deposit(testAmount);
        assertEq(styCRV.balanceOf(address(market.predictEscrow(user))), testAmount, "Escrow balance did not increase");
    ...
    function testDeposit2() public {
        ...
        assertEq(styCRV.balanceOf(address(market.predictEscrow(user))), 0, "User balance not 0");
        assertEq(styCRV.balanceOf(address(market.predictEscrow(user2))), testAmount, "User2 escrow balance did not increase ");
        assertEq(collateral.balanceOf(user), balanceUserBefore - testAmount, "User balance did not decrease");
    ...
    function testBorrowOnBehalf() public {
        ...
        assertEq(styCRV.balanceOf(address(market.predictEscrow(userPk))), testAmount, "failed to deposit collateral");
    ...
    function testLiquidate_Succeeds_WhenPriceFallBelowBorrowValuee() public {
        ...
        vm.startPrank(user, user);
        deposit(testAmount);
        maxBorrowAmount = market.getCreditLimit(user);
    ...
    function testGetWithdrawalLimit_Returns_CollateralBalanceAdjustedForDebts() public {
        ...
        vm.startPrank(user, user);
        deposit(testAmount);
        borrowAmount = market.getCreditLimit(user);
        market.borrow(borrowAmount);
        uint collateralBalance = market.escrows(user).balance();
    ...
    function testWithdraw() public {
        ...
        assertEq(styCRV.balanceOf(address(market.predictEscrow(user))), testAmount, "failed to deposit collateral");
        assertEq(collateral.balanceOf(user), 0, "failed to deposit collateral");

        market.withdraw(testAmount);

        assertEq(styCRV.balanceOf(address(market.predictEscrow(user))), 0, "failed to withdraw collateral");
    ...
    function testWithdraw_Fail_When_WithdrawingCollateralBelowCF() public {
        ...
        assertEq(styCRV.balanceOf(address(market.predictEscrow(user))), testAmount, "successfully withdrew collateral");
    ...
    function testWithdrawOnBehalf() public {
        ...
        assertEq(styCRV.balanceOf(address(market.predictEscrow(userPk))), 0, "failed to withdraw collateral");
