...
contract WbtcFeedFork is Test {
    ...
    function test_latestRoundData_NominalCaseWithinBounds() public {
        ...
        (, int256 btcToUsdPrice, , uint clUpdatedAt1,) = feed.btcToUsd().latestRoundData();
        (, int256 wbtcToBtcPrice, , uint clUpdatedAt2,) = feed.wbtcToBtc().latestRoundData();
        ...
        if(clUpdatedAt1 < clUpdatedAt2){
            (clRoundId, , clStartedAt, , clAnsweredInRound) = feed.btcToUsd().latestRoundData();
            assertEq(clUpdatedAt1, updatedAt);
        } else {
            (clRoundId, , clStartedAt, ,  clAnsweredInRound) = feed.wbtcToBtc().latestRoundData();
