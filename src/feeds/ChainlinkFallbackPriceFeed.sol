...
contract ChainlinkFallbackPriceFeed {
    ...
    constructor(address _owner, address _assetToUsd, address _assetToUsdFallback, uint256 _assetToUsdHeartbeat, uint8 _decimals) {
        owner = _owner;
     ...
     */
    function latestRoundData() public view returns (uint80 roundId, int256 usdPrice, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound){
        if(isPriceStale()){
            if(hasFallback()){
                (roundId, usdPrice, startedAt, updatedAt, answeredInRound) = assetToUsdFallback.latestRoundData();
                uint8 fallbackDecimals = assetToUsdFallback.decimals();
                if(fallbackDecimals > decimals){
                    usdPrice = usdPrice / int(10 ** (fallbackDecimals - decimals));
                } else if(fallbackDecimals < decimals) {
                    usdPrice = usdPrice * int(10 ** (decimals - fallbackDecimals));
                }
            } else {
                (roundId, usdPrice, startedAt, updatedAt, answeredInRound) = assetToUsd.latestRoundData();
                updatedAt = 0;
            }
        } else {
            (roundId, usdPrice, startedAt, updatedAt, answeredInRound) = assetToUsd.latestRoundData();
