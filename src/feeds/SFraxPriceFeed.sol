...
contract SFraxPriceFeed {
    error OnlyGov();

    IERC4626 public constant sFrax =
        IERC4626(0xA663B02CF0a4b149d2aD41910CB81e23e1c41c32);
    ...
    IChainlinkFeed public constant fraxToEth =
        IChainlinkFeed(0x14d04Fff8D21bd62987a5cE9ce543d2F1edF5D3E);

    uint256 public ethHeartbeat = 1 hours;
    uint256 public fraxToEthHeartbeat = 24 hours;
    ...
    /**
     * @notice Retrieves the latest round data for sFrax price feed
     * @dev This function calculates the sFrax price in USD by combining the Frax to USD price from a Chainlink oracle 
     * and the Frax per sFrax ratio from the sFrax vault contract. 
     * If Frax/USD price is out of bounds, it will fallback to ETH/USD and Frax/ETH oracles to get the price.
     * @return roundId The round ID of the Chainlink price feed
     * @return sFraxUsdPrice The latest sFrax price in USD
        ...
        returns (uint80, int256, uint256, uint256, uint80)
    {
        ...
        uint256 fraxPerShare = sFrax.convertToAssets(1 ether);

        if (isPriceOutOfBounds(fraxUsdPrice, fraxToUsd)) {
            (
                roundId,
                fraxUsdPrice,
                startedAt,
        ...
        int256 sFraxUsdPrice = int256(fraxPerShare) * fraxUsdPrice / 10 ** 8;

        return (roundId, sFraxUsdPrice, startedAt, updatedAt, answeredInRound);
    }
    ...
    */
    ...
    /**
     * @notice Fetches the ETH to USD price and the Frax/ETH to get Frax/USD price, adjusts the decimals to match Chainlink oracles.
     ...
     * @return fraxToUsdPrice The latest Frax price in USD computed from the ETH/USD and Frax/ETH feeds
        ...
        returns (uint80, int256, uint256, uint256, uint80)
    {
        ...
        (
            ,
            int256 fraxToEthPrice,
            ,
            uint256 updatedFraxEth,
        ) = fraxToEth.latestRoundData();

        int256 fraxToUsdPrice = ethToUsdPrice * fraxToEthPrice / 10**18;
        ...
        if(isPriceOutOfBounds(fraxToEthPrice, fraxToEth) || block.timestamp - updatedFraxEth > fraxToEthHeartbeat) {
            // Will force stale price on borrow controller
            updatedAt = 0;
        }
     ...
     */
    function setFraxEthHeartbeat(uint256 newHeartbeat) external onlyGov {
        fraxToEthHeartbeat = newHeartbeat;
    }
