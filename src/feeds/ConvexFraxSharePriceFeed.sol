...
contract ConvexFraxSharePriceFeed is IChainlinkFeed {
     ...
     * @dev This function calculates the CvxFxs price in USD by combining the CRV to USD price from a Chainlink oracle and the CvxFxs to FXS ratio from a Curve pool
    ...
    function latestRoundData() external view returns (uint80, int256, uint256, uint256, uint80){
        ...
        if(fxsPerCvxFxs > 10 ** 18){
            //1 CRV can always be traded for 1 CvxFxs, so price for CvxFxs should never be higher than the price of CRV
            fxsPerCvxFxs = 10**18;
    ...
    /**
     * @notice Sets a new minimum CRV per CvxFxs ratio
     * @dev Can only be called by the gov or guardian addresses
     * @param newMinRatio The new minimum CRV per CvxFxs ratio
     */
